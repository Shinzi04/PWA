import { convertBase64ToUint8Array } from './converter';
...
export async function subscribe() {
  ...
  try {
    ...
    const response = await StoryAPI.subscribeToStoryNotification({ endpoint, keys });
...
export async function unsubscribe() {
  ...
  try {
    ...
    if (!unsubscribed) {
      alert(failureUnsubscribeMessage);
      await StoryAPI.unsubscribeToStoryNotification({ endpoint, keys });
      return;
    }
